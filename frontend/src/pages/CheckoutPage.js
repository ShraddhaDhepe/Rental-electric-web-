...
const CheckoutPage = () => {
  ...
  const handleRazorpayPayment = async () => {
    try {
      ...
      //  - raw:      { id, amount, currency, ... }  (Razorpay's object directly)
      const rpOrder = rpResponse?.order?.id ? rpResponse.order : rpResponse;
      const rpKey = rpResponse?.key || RAZORPAY_KEY;

      if (!rpKey) {
        toast.error('Razorpay key missing. Set VITE_RAZORPAY_KEY_ID in your frontend .env and redeploy.');
        setRazorpayLoading(false);
        return;
      }
      ...
      const options = {
        key: rpKey, // works whether backend returns it or we fall back to env var
